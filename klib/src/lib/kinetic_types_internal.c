...
bool Copy_KineticProto_KeyValue_to_KineticEntry(KineticProto_KeyValue* keyValue, KineticEntry* entry)
{
    ...
    if (keyValue != NULL && entry != NULL) {
        ...
        /* has_dbVersion could be true with no db_Version data */
        if (keyValue->has_dbVersion && keyValue->dbVersion.len)
        {
        ...
        if (keyValue->has_key && keyValue->key.len)
        {
        ...
        if (keyValue->has_tag && keyValue->tag.len)
        {

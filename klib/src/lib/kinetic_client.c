...
void    KineticClient_InternalWait(KineticOperation * operation)
{
		...
		pthread_mutex_unlock(&operation->callback_mutex);
}
...
KineticStatus KineticClient_Init(const char *logFile, int logLevel, KineticHeartbeatCallback callback)
{
	...
	if (((status = Heartbeat_Init(callback)) != KINETIC_STATUS_SUCCESS) ||
		((status =  KineticLogger_Init(logFile, logLevel)) != KINETIC_STATUS_SUCCESS))
